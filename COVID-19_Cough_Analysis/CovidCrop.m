...
%This is the code used to separate the Covid-19 coughs from the original
%signal. Works with the Covid folder
clc
sAudioFiles = dir(fullfile('C:\Program Files (x86)\University Year 3\Semester 2\Biomedical Signals, Systems and Control\Assignment\Project\Covid'));  
...
for iCount = 3:10
    ...
    for iCount2 = 1:iLength
        if D(1,iCount2) == 1
                ...
                Filename = sprintf('covid%i_%i_crop.wav',iCount,iCount2-iCounter);
               audiowrite(Filename,no_covidCrop,n_fs); 
               
                subplot(2,2,iCount2-iCounter);
                plot(t_crop,no_covidCrop)
                titlename = sprintf('Covid Positive Patient %i', iCount -2);
                title(titlename);
                xlabel('Time (s)');
                ylabel('Amplitude');
        else
